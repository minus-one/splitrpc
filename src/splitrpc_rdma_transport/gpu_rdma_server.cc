...
//////////////////////////////// BUF-MON End
  ...
  for(int16_t i = 0 ; i < get_ort_batch_size(); i++) {
  ...
  int rr_ci_idx = 0;
  P2pRpcAppRr *ci_app_rr = app_rr_pool->get_app_rr(rr_ci_idx);
  //ci_app_rr->print_app_rr_info();
  //TRACE_PRINTF("Waiting on rr_ci_idx: %d, app_rr: %p, h_state: %p, val: %d\n", 
  //    rr_ci_idx, (void*)ci_app_rr, (void*)ci_app_rr->h_state, *ACCESS_ONCE(ci_app_rr->h_state));
  ...
  while(ACCESS_ONCE(force_quit) == 0) {
    if(*ACCESS_ONCE(ci_app_rr->h_state) == APP_RR_STATUS::RX_COMPLETE) {
#ifdef PROFILE_MODE
      DwStartNs = getCurNs();
#endif
      TRACE_PRINTF("Got RX_COMPLETE rr_ci_idx: %d, AppRr: %p\n", 
          rr_ci_idx, (void*)ci_app_rr);
      app_server->do_work_sync(0, rr_ci_idx);

      // Notify that work is complete
      *ACCESS_ONCE(ci_app_rr->h_state) = APP_RR_STATUS::WORK_COMPLETE;
      ...
      //ret = post_send_mr_sync(rctx, req_mon_mr);

      // Post-processing, Regenerate the AppRr item
      app_rr_pool->get_next();
      TRACE_PRINTF("WORK_COMPLETE sent rr_ci_idx: %d\n", rr_ci_idx);
      rr_ci_idx = (rr_ci_idx + 1) % app_rr_pool_size;
      ci_app_rr = app_rr_pool->get_app_rr(rr_ci_idx); 
      TRACE_PRINTF("Will next wait for AppRr: %p, rr_ci_idx: %d, h_state: %p\n", 
          (void*)ci_app_rr, rr_ci_idx, (void*)ci_app_rr->h_state);
#ifdef PROFILE_MODE
      DwDelay.push_back(getCurNs() - DwStartNs);
#endif
      //ci_app_rr->print_app_rr_info();
      //TRACE_PRINTF("Waiting on rr_ci_idx: %d, app_rr: %p, h_state: %p, val: %d\n", 
      //    rr_ci_idx, (void*)ci_app_rr, (void*)ci_app_rr->h_state, *ACCESS_ONCE(ci_app_rr->h_state));
  ...
  printf("Exited worker loop, rr_ci_idx: %d, ci_app_rr: %p, h_state: %p, h_state_val: %d\n",
      rr_ci_idx, (void*)ci_app_rr, (void*)ci_app_rr->h_state, *ACCESS_ONCE(ci_app_rr->h_state));
  PROF_PRINT("DwDelay: ", DwDelay);
  ret = disconnect(rctx);
  if (ret) { 
